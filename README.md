...
To create tests, add `test()` (or `test()`) blocks with the name of the test and its code. You may optionally wrap them in `describe()` blocks for logical grouping but this is neither required nor recommended.
...
You can replace `test()` with `xtest()` to temporarily exclude a test from being executed.<br>
Similarly, `ftest()` lets you focus on a specific test without running any other tests.
